...
class AlienBehavior(EnemyBehavior):
    ...
    Tipo: Usa A* para perseguir al jugador de forma inteligente
    """
    ...
    def _create_behavior_tree(self):
        ...
        # Secuencia principal: Siempre perseguir usando A*
...
class GhostBehavior(EnemyBehavior):
    ...
    def _create_behavior_tree(self):
        ...
        # Acción de vagar (cuando el jugador no está cerca)
        wander_action = Action(self._wander, "Wander")
        
        # Ensamblar el árbol
        root_selector.add_child(flee_sequence)
        root_selector.add_child(wander_action)
...
class ZombieBehavior(EnemyBehavior):
    ...
    Tipo: Se mueve de forma completamente aleatoria
    """
    
    def __init__(self, enemy_data, pathfinder, player_pos_getter, all_enemies=None):
        super().__init__(enemy_data, pathfinder, player_pos_getter, all_enemies)
        self.last_move_time = 0
        self.move_delay = 0.8  # Zombies son lentos
        self._create_behavior_tree()
        
    def _create_behavior_tree(self):
        """Crea el árbol de comportamiento para el zombie"""
        
        # Secuencia principal con delay
        root_sequence = Sequence("ZombieMainSequence")
        
        # Condición: ¿Puede moverse ahora? (zombie lento)
...
class VillainBehavior(EnemyBehavior):
    ...
    Tipo: Mezcla entre persecución y emboscada
    """
    ...
    def _should_ambush(self, blackboard):
        ...
        # Cambiar de modo cada 5 segundos
        if current_time - self.mode_switch_time > 5.0:
            self.ambush_mode = not self.ambush_mode
            self.mode_switch_time = current_time
            
        return self.ambush_mode and distance > 3.0
        
    def _should_chase(self, blackboard):
        """Decide si debe perseguir directamente"""
        distance = blackboard["distance_to_player"]
        return not self.ambush_mode and distance <= 6.0
    ...
    def _setup_ambush(self, blackboard):
        ...
        try:
            ...
            # Predecir donde estará el jugador
            predicted_positions = [
                [player_pos[0] + 3, player_pos[1]],
                [player_pos[0] - 3, player_pos[1]],
                [player_pos[0], player_pos[1] + 3],
                [player_pos[0], player_pos[1] - 3]
            ]
...
class DemonBehavior(EnemyBehavior):
    ...
    Tipo: Se teletransporta en tramos
    """
    
    def __init__(self, enemy_data, pathfinder, player_pos_getter, all_enemies=None):
        super().__init__(enemy_data, pathfinder, player_pos_getter, all_enemies)
        self.last_teleport_time = 0
        self.teleport_cooldown = 3.0  # 3 segundos entre teletransportes
        self._create_behavior_tree()
    ...
    def _create_behavior_tree(self):
        ...
        normal_move = Action(self._normal_move, "NormalMove")
        
        # Ensamblar el árbol
        root_selector.add_child(teleport_sequence)
        root_selector.add_child(normal_move)
    ...
    def _teleport_near_player(self, blackboard):
        ...
        try:
            ...
            # Posiciones cercanas al jugador para teletransportarse
            teleport_positions = []
            for radius in range(2, 5):  # Anillo alrededor del jugador
...
class ClownBehavior(EnemyBehavior):
    ...
    Tipo: Deja trampas y se mueve en patrones erráticos
    """
    ...
    def __init__(self, enemy_data, pathfinder, player_pos_getter, all_enemies=None):
        ...
        self.trap_cooldown = 4.0  # 4 segundos entre trampas
    ...
    def _create_behavior_tree(self):
        ...
        # Acción de movimiento errático
        erratic_move = Action(self._erratic_movement, "ErraticMovement")
        
        # Ensamblar el árbol
        root_selector.add_child(trap_sequence)
        root_selector.add_child(erratic_move)
    ...
    def _place_trap(self, blackboard):
        ...
        try:
            ...
            # Limpiar trampas viejas (después de 10 segundos)
            self.traps = [trap for trap in self.traps 
                         if current_time - trap['time'] < 10.0]
    ...
    def _erratic_movement(self, blackboard):
        ...
        try:
            ...
            # Cambiar patrón cada 5 pasos
            if self.pattern_steps >= 5:
